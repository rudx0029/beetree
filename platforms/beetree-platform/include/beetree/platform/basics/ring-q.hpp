...
{
    ///
    /// C++ wrapper around the 'C' bte_ringq.
    ///
    ///
    class RingQ
    {
    public:
        ...
        ///
        RingQ(uint8_t* buff, std::size_t n) { bte_ringq_init(buff, n, &m_q); }
        ...
        ///
        uint8_t pop() { return bte_ringq_pop(&m_q); }

        ///
        /// Push the given `byte` onto the ring queue.
        ///
        /// @note
        /// Invoking `push()` is undefined when the number of elements in the Q
        ...
        ///
        void push(uint8_t byte) { bte_ringq_push(&m_q, byte); }

        ///
        /// @return the number of elements currently in the ring 'Q'
        ///
        std::size_t count() const { return bte_ringq_count(&m_q); }

        ///
        /// @param q  the ring Q context
        /// @return the ring Q's capacity, i.e. the size of the underlying buffer
        ///
        std::size_t capacity() const { return bte_ringq_capacity(&m_q); }
